...
unless a field is excluded explicitly through `excludePaths`:
+
...
i.e. filter cannot reference a path that, in general is there but, was made unreachable by a filter at a lower level that
excluded at least one of this path's nodes (implicitly or explicitly).
====

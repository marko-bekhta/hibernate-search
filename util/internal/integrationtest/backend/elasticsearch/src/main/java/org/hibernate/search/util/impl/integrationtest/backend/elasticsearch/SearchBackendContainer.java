...
public final class SearchBackendContainer {
	...
	private static GenericContainer<?> elasticsearch(String name, String tag, ElasticsearchVersion version) {
		...
		if ( version.majorOptional().orElse( Integer.MIN_VALUE ) == 7 ) {
			...
			if ( version.minor().orElse( Integer.MAX_VALUE ) > 8 ) {
				container.withEnv( "stack.templates.enabled", "false" );
			}
			if ( version.minor().orElse( Integer.MAX_VALUE ) > 5 ) {
				container.withEnv( "indices.lifecycle.history_index_enabled", "false" );
			}
			return container.withEnv( "xpack.ml.enabled", "false" )
					.withEnv( "xpack.watcher.enabled", "false" );
		}

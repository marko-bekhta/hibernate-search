...
final class ParameterizedClassUtils {
	...
	static boolean isParameterizedSetup(Method candidate) {
		if ( isAnnotated( candidate, ParameterizedSetup.class ) ) {
			return true;
		}

		if ( !isActualTestMethodToExecute( candidate )
				&& ( isAnnotated( candidate, TestTemplate.class ) || isAnnotated( candidate, TestFactory.class ) ) ) {

			throw new IllegalStateException(
					"This type of tests are not yet supported by the parameterized class test extension."
							+ " Remove any JUnit @TestTemplate/@TestFactory annotations from " + candidate );
		}

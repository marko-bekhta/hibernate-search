...
final class ParameterizedClassExtension
					...
					public TestTemplateInvocationContext next() {
						if ( envInitialized ) {
							...
							return new ParameterizedClassTestTemplateInvocationContext(
									testMethod.getName(),
									MethodInterceptor.TEST_EXECUTION_INTERCEPTOR
							);
						}
						else {
							return new ParameterizedClassTestTemplateInvocationContext(
									"Setup Parameters: " + Arrays.toString( envArguments.get( envIndex ) ),
									MethodInterceptor.SETUP_EXECUTION_INTERCEPTOR
							);
						}
					}
				}, Spliterator.NONNULL
	...
	private class ParameterizedClassTestTemplateInvocationContext implements TestTemplateInvocationContext {
		...
		public String getDisplayName(int invocationIndex) {
			return "Configuration #" + envIndex + ": " + name;
		}
	...
	private static class MethodInterceptor implements InvocationInterceptor {
		...
		private static final MethodInterceptor SETUP_EXECUTION_INTERCEPTOR = new MethodInterceptor( true );
		private static final MethodInterceptor TEST_EXECUTION_INTERCEPTOR = new MethodInterceptor( false );
